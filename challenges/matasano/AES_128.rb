...
# Author: Ben Pottier
#
# Very basic Ruby implementation of AES-128, ECB and CBC
# 
# Exposes: 
...
class AES_128
  ...
  # CONSTANTS - GALOIS LOOKUP TABLES
  GALOIS_2 =  [0x00,0x02,0x04,0x06,0x08,0x0a,0x0c,0x0e,0x10,0x12,0x14,0x16,0x18,0x1a,0x1c,0x1e,
  ...
  GALOIS_3 =  [0x00,0x03,0x06,0x05,0x0c,0x0f,0x0a,0x09,0x18,0x1b,0x1e,0x1d,0x14,0x17,0x12,0x11,
  ...
  GALOIS_9 =  [0x00,0x09,0x12,0x1b,0x24,0x2d,0x36,0x3f,0x48,0x41,0x5a,0x53,0x6c,0x65,0x7e,0x77,
  ...
  GALOIS_11 = [0x00,0x0b,0x16,0x1d,0x2c,0x27,0x3a,0x31,0x58,0x53,0x4e,0x45,0x74,0x7f,0x62,0x69,
  ...
  GALOIS_13 = [0x00,0x0d,0x1a,0x17,0x34,0x39,0x2e,0x23,0x68,0x65,0x72,0x7f,0x5c,0x51,0x46,0x4b,
  ...
  GALOIS_14 = [0x00,0x0e,0x1c,0x12,0x38,0x36,0x24,0x2a,0x70,0x7e,0x6c,0x62,0x48,0x46,0x54,0x5a,
  ...
  def self.decrypt_CBC(buffer, key, iv)
    ...
    buffer.each_slice(16) do |block| 
      ...
      message += buffer_XOR(message_block, iv).pack('C*') 
      iv = block
    end
    message
  end
  ...
  def self.encryptAES_128(buffer, key)
    ...
    # Initial round
    state = add_round_key(buffer, key, 0)
    # 9 rounds
    9.times do |i|
      state = sbox_bytes(state)
      state = shift_rows(state)
      state = mix_columns(state)
      state = add_round_key(state, key, i+1)
    end
    # Final round
    state = sbox_bytes(state)
    state = shift_rows(state)
  ...
  def self.decryptAES_128(buffer, key)
    ...
    # Inverse final round
    state = add_round_key(buffer, key, 10)
    state = shift_rows_inverse(state)
    state = sbox_bytes_inverse(state)
    # 9 rounds inverse
    9.times do |i|
      state = add_round_key(state, key, 9-i)
      state = mix_columns_inverse(state)
      state = shift_rows_inverse(state)
      state = sbox_bytes_inverse(state)
    end
    # Inverse of first round
  ...
  def self.mix_column_inverse(bytes)
    ...
    [ GALOIS_14[b1] ^ GALOIS_9[b4] ^ GALOIS_13[b3] ^ GALOIS_11[b2], 
      ...
      GALOIS_14[b3] ^ GALOIS_9[b2] ^ GALOIS_13[b1] ^ GALOIS_11[b4], 
      GALOIS_14[b4] ^ GALOIS_9[b3] ^ GALOIS_13[b2] ^ GALOIS_11[b1] ] 
  end
  ...
  # call buffer_XOR with the appropriate part of the key
  def self.add_round_key(buffer, key, i)
    buffer_XOR(buffer, key[i*16, 16])
  end
  ...
  def self.expand_key(key)
    ...
    r = 1 # rcon starts at 1
    until key.size == b # iterate until we have our wanted amount of bytes
      t = key[-4, 4] # take the last 4 bytes from our key
      t = key_schedule_core(t, r) # run key schedule core across the 4 bytes with our value
      r += 1 # increment rcon
      t = buffer_XOR(t, key[-n, 4]) # as per wiki: 'exclusive-or t with the four-byte block n bytes before the new expanded key'
      key += t # add the 4 bytes to the key
      3.times do # next 12 bytes 
        t = key[-4, 4] # take last 4 bytes again
        t = buffer_XOR(t, key[-n, 4]) # aaaand xor the 4 bytes n bytes before the new key
        key += t
      end
      # that's it for 128 bit
      key
    end
    key
  end
